    ...
    def process_text(text: str, stress_predictor) -> 'Markup':
        ...
        for text_line in text_lines:
            ...
            for token in tokens:
                ...
                # если это предлог без гласной, то игнорируем его
                if len(word.syllables) == 1 and len(stresses) == 1 and stresses[0] == -1:
                    continue
        ...
        for i, ch in enumerate(string):
            ...
            for state in current_states:
                ...
                for variant in variants:
                    ...
                    strong_errors = state.strong_errors + int(variant.text.isupper() and variant.text != ch and ch != 'A')
                    weak_errors = state.weak_errors + int(variant.text.islower() and variant.text != ch.lower() and ch != 'A')
            ...
            if len(new_states) == 0:
                ...
                diff = (len(string) - i)
                return pattern, strong_errors + diff, weak_errors, True
    ...
    def __get_line_pattern(line: Line) -> str:
        ...
        for w, word in enumerate(line.words):
            if len(word.syllables) == 0:
                pattern += "U"
                print(word.text, 'len syllables=0')
